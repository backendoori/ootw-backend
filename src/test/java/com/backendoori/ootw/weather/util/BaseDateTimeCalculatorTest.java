...
class BaseDateTimeCalculatorTest {
    ...
    private static Stream<Arguments> provideVillageForecastRequestDateTime() {
        return Stream.of(
            Arguments.of(LocalDateTime.of(2023, 12, 25, 0, 12), "20231224", "2312"),
            Arguments.of(LocalDateTime.of(2024, 1, 8, 23, 0), "20240108", "0200"),
            Arguments.of(LocalDateTime.of(2028, 5, 30, 1, 1), "20280529", "2301"));
    }

    private static Stream<Arguments> provideUltraShortForecastRequestDateTime() {
        return Stream.of(
            Arguments.of(LocalDateTime.of(2023, 12, 25, 0, 12), "20231224", "2312"),
    ...
    void testGetCurrentBaseDateTime(LocalDateTime localDateTime, String baseDate, String baseTime) {
        // given
    ...
    @ParameterizedTest(name = "[{index}] 시간이 {0}이면 일자는 {1}, 시간은 {2} 형식이 된다.")
    @MethodSource("provideUltraShortForecastRequestDateTime")
    @DisplayName("현재시간을 baseDate/baseTime 형식으로 바꿔주고, 초단기예보 발표 시간에 맞게 계산이 된다.")
    void testGetUltraShortForecastRequestBaseDateTime(LocalDateTime localDateTime, String baseDate, String baseTime) {
        // given
        BaseDateTime baseDateTime = new BaseDateTime(baseDate, baseTime);
    ...
    @ParameterizedTest(name = "[{index}] 시간이 {0}이면 일자는 {1}, 시간은 {2} 형식이 된다.")
    @MethodSource("provideVillageForecastRequestDateTime")
    @DisplayName("현재시간을 baseDate/baseTime 형식으로 바꿔주고, 단기예보 발표 시간에 맞게 계산이 된다.")
    void testGetVillageForecastRequestBaseDateTime(LocalDateTime localDateTime, String baseDate, String baseTime) {
        // given
        BaseDateTime baseDateTime = new BaseDateTime(baseDate, baseTime);

        // when // then
        assertThat(BaseDateTimeCalculator.getVillageForecastRequestBaseDateTime(localDateTime)).isEqualTo(
            baseDateTime);
    }
